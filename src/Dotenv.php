...
final class Dotenv
{
     ...
     */
    public function load(string $path, string ...$extraPaths)
    {
    ...
    public function loadEnv(
        string $path, string $varName = 'APP_ENV', string $defaultEnv = 'dev', array $testEnvs = ['test']
    ) {
     ...
     * @throws PathException   when a file does not exist or is not readable
     */
    public function overload(string $path, string ...$extraPaths)
    {
     ...
     */
    public function populate(array $values, bool $overrideExistingVars = false)
    {
    ...
    private function resolveCommands(string $value): string
    {
        ...
        return preg_replace_callback($regex, function ($matches) {
            ...
            $process = method_exists(Process::class,
                                     'fromShellCommandline') ? Process::fromShellCommandline('echo ' . $matches[0]) : new Process('echo ' . $matches[0]);
            ...
            try {
                ...
                throw $this->createFormatException(sprintf('Issue expanding a command (%s)',
        ...
        $value = preg_replace_callback($regex, function ($matches) {
            ...
            if ('' === $value && isset($matches['default_value'])) {
                ...
                if (false !== $unsupportedChars) {
                    throw $this->createFormatException(sprintf('Unsupported character "%s" found in the default value of variable "$%s".',
                                                               $unsupportedChars[0], $name));
                }
    ...
    private function createFormatException(string $message): FormatException
    {
        return new FormatException($message,
                                   new FormatExceptionContext($this->data, $this->path, $this->lineno, $this->cursor));
    }

    private function doLoad(bool $overrideExistingVars, array $paths)
    {
